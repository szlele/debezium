...
public final class TestHelper {
    ...
    public static PostgresConnectorConfig.LogicalDecoder decoderPlugin() {
        ...
        return (s == null || s.length() == 0) ? PostgresConnectorConfig.LogicalDecoder.MPPDB_DECODING : PostgresConnectorConfig.LogicalDecoder.parse(s);
    }
    ...
    public static void dropAllSchemas() throws SQLException {
        ...
        if (schemaNames.contains(PostgresSchema.PUBLIC_SCHEMA_NAME)) {
            schemaNames.remove(PostgresSchema.PUBLIC_SCHEMA_NAME);
        }
    ...
    protected static Set<String> schemaNames() throws SQLException {
        try (PostgresConnection connection = create()) {
            return connection.readAllSchemaNames(
                    ((Predicate<String>) Arrays.asList("pg_catalog", "information_schema", "dbe_perf", "sqladvisor", "dbe_pldebugger", "cstore", "blockchain",
                            "pkg_service", "snapshot", "dbe_sql_util", "dbe_lob", "dbe_xmldom", "resource_manager", "dbe_pldeveloper", "dbe_ilm_admin",
                            "dbe_file")::contains)
                                    .negate());
        }
    }
