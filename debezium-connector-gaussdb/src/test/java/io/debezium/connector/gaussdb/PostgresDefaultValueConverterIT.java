...
public class PostgresDefaultValueConverterIT extends AbstractConnectorTest {
    ...
    private void assertDefaultValueChangeRecord(SourceRecord sourceRecord) {
        ...
        Assertions.assertThat(((Struct) sourceRecord.value()).getStruct("after").getInt64("dt2")).isNotNull();
        ...
        Assertions.assertThat(valueSchema.field("after").schema().field("dt2").schema().defaultValue()).isEqualTo(0L);
        Assertions.assertThat(valueSchema.field("after").schema().field("dt3").schema().defaultValue()).isNull();
    }
}
