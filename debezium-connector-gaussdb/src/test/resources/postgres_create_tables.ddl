...
CREATE TABLE IF NOT EXISTS numeric_table (pk SERIAL, si SMALLINT, i INTEGER, bi BIGINT,
...
CREATE TABLE IF NOT EXISTS numeric_decimal_table (pk SERIAL,
...
CREATE TABLE IF NOT EXISTS network_address_table (pk SERIAL, i INET, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS cidr_network_address_table (pk SERIAL, i CIDR, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS macaddr_table(pk SERIAL, m MACADDR, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS cash_table (pk SERIAL, csh MONEY, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS bitbin_table (pk SERIAL, ba BYTEA, bol BIT(1), bol2 BIT, bs BIT(2), bs7 BIT(7), bv BIT VARYING(2), bv2 BIT VARYING(24), bvl BIT VARYING(64), bvunlimited1 BIT VARYING, bvunlimited2 BIT VARYING, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS bytea_binmode_table (pk SERIAL, ba BYTEA, PRIMARY KEY(pk));

CREATE TABLE IF NOT EXISTS time_table (pk SERIAL, ts TIMESTAMP, tsneg TIMESTAMP(6) WITHOUT TIME ZONE, ts_ms TIMESTAMP(3), ts_us TIMESTAMP(6), tz TIMESTAMPTZ, date DATE,
    date_pinf DATE,
    date_ninf DATE,
...
CREATE TABLE IF NOT EXISTS hstore_table (pk serial, hs hstore, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS hstore_table_mul (pk serial, hs hstore, hsarr hstore[], PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS hstore_table_with_null (pk serial, hs hstore, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS hstore_table_with_special (pk serial, hs hstore, PRIMARY KEY(pk));
CREATE TABLE IF NOT EXISTS circle_table (pk serial, ccircle circle, PRIMARY KEY(pk));
...
CREATE TABLE IF NOT EXISTS table_without_pk(id serial, val INTEGER);
DROP SCHEMA IF EXISTS "Quoted_"" . Schema" CASCADE;
CREATE SCHEMA IF NOT EXISTS
    "Quoted_"" . Schema";
-- GRANT ALL ON ALL TABLES IN SCHEMA "Quoted_Schema" TO postgres;
CREATE TABLE IF NOT EXISTS "Quoted_"" . Schema"."Quoted_"" . Table" (pk SERIAL, "Quoted_"" . Text_Column" TEXT, PRIMARY KEY(pk));
